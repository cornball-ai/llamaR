...
for (tool_name in skill_names) {
    ...
    # Formals vs properties: every model-owned formal should be in properties;
    # server-side context and R evaluation environments are capability
    # boundaries, not JSON inputs.
    fml_names <- setdiff(names(formals(fn)), c("...", "ctx", "envir"))
